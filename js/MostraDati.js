...
export async function mostraDatiLivello2(attivi, layers, livello, data, label, config, zoom) {
    ...
    for (const [nomeRegione, dettagli] of Object.entries(data[livello])) {
        ...
        // centro calcolato una sola volta
        if (!feature._center) {
            ...
            if (layers[i] && perc > 0) {
                if (comp)
                    elemento = creaElementoGraficoComparabile(
                        feature, perc, valore, label[`labelv${i + 1}`], i, config
                    );
                else
                    elemento = creaElementoGrafico(
                        feature, perc, i, config, livello, zoom
                    );

                elemento.bindTooltip(
                    `<b>${feature.properties.name}</b><br>Serie ${i + 1}: ${perc}% <br>v:${valore} ${label[`labelv${i + 1}`]}`
                );
...
/* Il contorno viene marcato con l'opzione "contorno": permette al livello
 * inferiore (Punti.js) di rimuovere i soli marker mantenendo i confini disegnati.
 */
function contorno(feature, fill, colore) {
    ...
    geojsonLayer.options.contorno = true;
    return geojsonLayer;
}
